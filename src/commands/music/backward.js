...
export default {
  data: new SlashCommandBuilder()
    .setName('backward')
    .setDescription('Skip backward in the current track')
  ...
  async execute(interaction) {
    try {
      ...
      logger.info(`Skipped backward ${seconds}s in guild ${interaction.guild.id}`);
      ...
      return interaction.reply({
        embeds: [createSuccessEmbed(
          '⏪ Skipped Backward',
          `Jumped **${seconds}s** back\n\n${EMOJIS.HEADPHONES} New position: \`${newPosFormatted}\` / \`${durationFormatted}\``
        )]
      });

    } catch (error) {
      logger.error('Backward command error:', error);
      return interaction.reply({
        embeds: [createErrorEmbed('Failed to skip backward', error.message)],
        ephemeral: true
      });
    }
  }
